...
export default function RegulatorHospitalAIAnalysisPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // 🛑 FIX: Deduplicate signals in case the Python AI backend accidentally inserted duplicates
      const uniqueSignals = (signalData || []).reduce((acc: AnomalySignal[], current) => {
        const isDuplicate = acc.find(
          item => item.signal_type === current.signal_type && item.explanation === current.explanation
        );
        if (!isDuplicate) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 font-medium animate-pulse">Loading TIIE Forensic Report...</p>
      </div>
    );
  }
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <section className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm min-h-[400px]">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Triggered Anomalies</h3>
                  <p className="text-sm text-slate-500">Only dimensions with detected flags will appear here.</p>
                </div>
